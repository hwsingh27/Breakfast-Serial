...
void Init_UART0(uint32_t baud_rate)
{
	...
	// Disable RX Input Active Edge Interrupt and LIN Break Detect Interrupt,\
	Select 2-Stop bits by setting SBNS field
	...
	// Transmit data not inverted, OR interrupt disabled, NF interrupt disabled,\
	FE interrupt disabled, PF interrupt disabled
...
void UART0_IRQHandler(void)
{
	...
	//check if hardware interrupt for TDRE is enabled, if enabled then\
	perform dequeue operation from Txbuf (transmitter buffer)
